...
If ::cTpNF == "E"
	...
	dbSeek(xFilial("SF1")+::cDoc+::cSerie+::cCliFor+::cLoja)
	If SF1->F1_TIPO $ "D/B"
		cAliasC := "SA1"
		cTipoCli := "Cliente"		
	Else
...
Else
	...
	dbSeek(xFilial("SF2")+::cDoc+::cSerie+::cCliFor+::cLoja)
	If SF2->F2_TIPO $ "D/B"
		cAliasC := "SA2"
		cTipoCli := "Fornecedor"
	Else
	...
	EndIf
EndIf
cCli := ::cCliFor + "/" + ::cLoja + " - " + Posicione(cAliasC,1,xFilial(cAliasC)+::cCliFor+::cLoja,IIF(cAliasC=="SA2","A2_NOME","A1_NOME"))
...
oGrp1      := TGroup():New( 016,004,052,336,"",oDlg1,CLR_BLACK,CLR_WHITE,.T.,.F. )
oSay1      := TSay():New( 028,012,{||"Nota Fiscal"},oGrp1,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,032,008)
oSay2      := TSay():New( 040,012,{||cTipoCli},oGrp1,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,032,008)
oSay3      := TSay():New( 028,048,{||cNF },oGrp1,,oFont1,.F.,.F.,.F.,.T.,CLR_BLUE,CLR_WHITE,060,008)
oSay4      := TSay():New( 040,048,{||cCli},oGrp1,,oFont1,.F.,.F.,.F.,.T.,CLR_BLUE,CLR_WHITE,280,008)
...
Enchoice(cAliasE,nRegE,nOpcE,/*aCRA*/,/*cLetra*/,/*cTexto*/,aCpoEnch,aPos,aAlterEnch,nModelo,/*nColMens*/,/*cMensagem*/,/*cTudoOk*/,oGrp3,lF3,lMemoria,lColumn,/*caTela*/,lNoFolder,lProperty)

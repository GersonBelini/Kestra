...
#DEFINE MAXVALORC  011                                                // Máximo de caracteres por linha de valores numéricos//Personalizado
...
For nZ := 1 To nLenDet
	...
	/*
	Inicio - Personalizado - André Oquendo TOTVSIP - 27/072015
	Imprimir a descrição do cadastro de produto para sair com os caracteres especiais.
	*/
	dbSelectArea("SB1")
	dbSetOrder(1)
	If dbSeek(xFilial("SB1")+padR(alltrim(oDet[nX]:_Prod:_cProd:TEXT),tamSX3("B1_COD")[1]," "))
		If oNF:_INFNFE:_DEST:_ENDERDEST:_UF:TEXT == "EX" .AND. oIdent:_TpNf:Text == "1"			
			oDet[nX]:_Prod:_xProd:TEXT := alltrim(SB1->B1_ZZCODEX)
		Else
			oDet[nX]:_Prod:_xProd:TEXT := alltrim(SB1->B1_DESC)
		EndIf
	EndIf	
	/*
	Fim - Personalizado - André Oquendo TOTVSIP - 27/072015
	Imprimir a descrição do cadastro de produto para sair com os caracteres especiais.
	*/

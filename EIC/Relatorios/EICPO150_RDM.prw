...
#DEFINE LITERAL_ALTERACAO          IF( nIdioma == INGLES, "REVISION Number: ", "ALTERAÇÃO NÚMERO: " ) //"ALTERAÃ+O N·mero: "
#DEFINE LITERAL_DATA               IF( nIdioma == INGLES, "Date: "             , "DATA:" ) //"Data: "
#DEFINE LITERAL_PAGINA             IF( nIdioma == INGLES, "Page: "             , "Página:" ) //"Pßgina: "
#DEFINE LITERAL_FORNECEDOR         IF( nIdioma == INGLES, "SUPPLIER........: " , "FORNECEDOR......: " ) //"FORNECEDOR......: "
#DEFINE LITERAL_ENDERECO           IF( nIdioma == INGLES, "ADDRESS.........: " , "ENDEREÇO........: " ) //"ENDEREÃO........: "
...
#DEFINE LITERAL_COMISSAO           IF( nIdioma == INGLES, "COMMISSION......: " , "COMISSÃO.......: " ) //"COMISS+O........: "
...
#DEFINE LITERAL_EMISSAO            IF( nIdioma == INGLES, "ISSUE"              , "EMISSAO" ) //"Emissão"
#DEFINE LITERAL_ITEM               IF( nIdioma == INGLES, "IT"                 , "IT"   )    //"It"
#DEFINE LITERAL_INCOTERMS          IF( nIdioma == INGLES, "INCOTERMS"          , "INCOTERMS.......: "   )    //"INCOTERMS.......: "
#DEFINE LITERAL_TOTAL              IF( nIdioma == INGLES, "TOTAL"              , "TOTAL"   ) //"Total"
...
Local aRotina :=  { { STR0029,"AxPesqui"  , 0 , 1},; //"Pesquisar"
                    { LITERAL_EMISSAO,"Eval(bFuncao)", 0 , 0}} //"Emissao"###"ReEmissao"
...
&& Adicionado bairro - Andre Borin - TOTVS IP - 30/06/2015
c2EndSYT := c2EndSYT + IF( !Empty(SYT->YT_BAIRRO), Alltrim(SYT->YT_BAIRRO)+' - ', "" )
&& FIM
...
&& Adicionado Telefone - Andre Borin - TOTVS IP - 30/06/2015
If !Empty(SYT->YT_TEL_IMP)
	oFnt := aFontes:COURIER_20_NEGRITO
	pTipo := 1
	PO150BateTraco()
	
	oSend( oPrn, "Say",  Linha, 630, "PHONE: " + Alltrim(SYT->YT_TEL_IMP), aFontes:TIMES_NEW_ROMAN_12 )
EndIf
&& FIM
...
Linha := Linha+20
oSend( oPrn, "oFont", aFontes:COURIER_20_NEGRITO)
&& TRACO_NORMAL //Comentado por Andre Borin - TOTVS IP - 30/06/2015
Linha := Linha+20
...
&& oSend( oPrn, "Say",  Linha, 100 , STR0040, aFontes:COURIER_12_NEGRITO ) //"PROFORMA INVOICE: " //Comentado por Andre Borin - TOTVS IP - 30/06/2015
&& oSend( oPrn, "Say",  Linha, 1720, LITERAL_DATA            , aFontes:COURIER_12_NEGRITO ) 	  //Comentado por Andre Borin - TOTVS IP - 30/06/2015
...
oSend( oPrn, "Say",  Linha, 100, LITERAL_INCOTERMS , aFontes:COURIER_12_NEGRITO ) //"INCOTERMS.......: "
...
oSend( oPrn, "Say",  Linha,  065, LITERAL_ITEM           , aFontes:TIMES_NEW_ROMAN_10_NEGRITO ) //"IT"
...
//Alterado por André Oquendo - de 50 para 20
If Len(Alltrim(cNomeFantasia)) > 15

	oSend( oPrn, "Say",  Linha,1450, substr(AllTrim(SA2->A2_NREDUZ),1,15) + Space(2) + IF(EICLOJA(), LITERAL_STORE /*"Loja:"*/ + Alltrim(SA2->A2_LOJA),"") ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 ) //FDR - 06/01/12
		Linha := Linha + 30
	oSend( oPrn, "Say",  Linha,1300, substr(AllTrim(SA2->A2_NREDUZ),16,25) + Space(2) ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1) 
		Linha := Linha - 30
else
...
//        oSend( oPrn, "Say",  Linha, 480,AV_Justifica( MEMOLINE( cDescrItem,nTam,i ) ), aFontes:COURIER_NEW_10_NEGRITO )
	...
	&& Andre Borin - Totvs IP - 09/09/2015
	&& Adicionado informação sobre a NCM
    IF EMPTY( MEMOLINE( cDescrItem,nTam, i ) )
       IF nNumero == 1         
           If SW3->(FieldPos("W3_PART_N")) # 0 .And. !Empty(SW3->W3_PART_N)
              oSend( oPrn, "Say",  Linha, 480 , SW3->W3_PART_N + " - NCM: " + SW3->W3_TEC,  aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
              lPulaLinha := .T.
           Else
              If !Empty( SA5->A5_CODPRF )
                 oSend( oPrn, "Say",  Linha, 480 , SA5->A5_CODPRF + " - NCM: " + SW3->W3_TEC,  aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
                 lPulaLinha := .T.
              ElseIf Empty(SA5->A5_CODPRF)
              	 oSend( oPrn, "Say",  Linha, 480 , "NCM: " + SW3->W3_TEC,  aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
                 lPulaLinha := .T.
              Endif
           EndIf
           nNumero := nNumero+1
...
oSend( oPrn, "Say",  Linha,1570, LITERAL_TOTAL, aFontes:COURIER_08_NEGRITO ) //"TOTAL " //DFS - 28/02/11 - Posicionamento correto
nTLinha := Linha 
...
oSend( oPrn, "Say",  Linha, 1570 , LITERAL_TOTAL + ALLTRIM( SW2->W2_INCOTER )         , aFontes:COURIER_08_NEGRITO ) //"TOTAL "

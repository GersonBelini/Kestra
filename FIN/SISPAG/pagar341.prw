...
Do Case
    ...
    Case _cTipo == "PP001"	//  Agencia e Conta Corrente Favorecido
       ...
       If SA2->A2_BANCO == "341"
          ...
          _cRetorno += strzero(val(substr(_cConta,6,5)),5)+" "+substr(SA2->A2_ZZNUMDC,1,1)                               
       Else
          _cRetorno := strzero(val(substr(SA2->A2_AGENCIA,1,5)),5)+" "
          _cRetorno += strzero(val(substr(_cConta,1,12)),12)
          If len(alltrim(SA2->A2_ZZNUMDC)) > 1
            _cRetorno += strzero(val(substr(SA2->A2_ZZNUMDC,1,2)),2)
          Else
             _cRetorno += " "+substr(SA2->A2_ZZNUMDC,1,1)
          EndIf                                     
       EndIf
    ...
    Case _cTipo == "PT001"	                           
         ...
         If SEA->EA_MODELO == "16"    
           ...
           // _cRetorno := "02"                         
            ...
            _cRetorno += If(!Empty(SE2->E2_ZZCDREC),STRZERO(Val(SE2->E2_ZZCDREC),4),STRZERO(Val(SE2->E2_CODRET),4))
  
            // Posicao 024 a 024: Tp Inscricao  1-CPF /  2-CNPJ               
           If !Empty(SE2->E2_ZZCNPJC)
               _cRetorno += Iif (len(alltrim(SE2->E2_ZZCNPJC))>11,"2","1")
           Else               
              _cRetorno += "2"       
           EndIf
            ...
            If !Empty(SE2->E2_ZZCNPJC)
               _cRetorno += Strzero(Val(SE2->E2_ZZCNPJC),14)
            Else
               _cRetorno += Subs(SM0->M0_CGC,1,14)
            EndIf
                                                 
            // Posicao 039 a 046: Periodo Apuracao
            _cRetorno += GravaData(SE2->E2_ZZAPURA,.F.,5) 
            
            // Posicao 047 a 063: Referencia   
            _cRetorno +=  Strzero(Val(SE2->E2_ZZREFER),17)

            // Posicao 064 a 077: Valor Principal
            _cRetorno += Strzero(SE2->E2_SALDO*100,14)                               
            
            // Posicao 078 a 091: Multa             
            _cRetorno += STRZERO(SE2->E2_ZZMULTA*100,14)         
            
            // Posicao 092 a 105: Juros        
            _cRetorno += Strzero(SE2->E2_ZZJUROS*100,14)          

            // Posicao 106 a 119: Valor Total (Principal + Multa + Juros)
            _cRetorno += STRZERO((SE2->E2_SALDO+SE2->E2_ZZMULTA+SE2->E2_ZZJUROS)*100,14)           
            ...
            If !Empty(SE2->E2_ZZCNPJC)
               _cRetorno += Subs(SE2->E2_ZZCONTR,1,30)
         ...
         ElseIf (SEA->EA_MODELO == "17")   
            ...
            _cRetorno +=  STRZERO(Val(SE2->E2_ZZCDREC),4)  
                                                 
            // Posicao 024 a 029: Competencia   MMAAAA  
            _cRetorno += STRZERO(MONTH(SE2->E2_ZZAPURA),2)+STRZERO(YEAR(SE2->E2_ZZAPURA),4)
           ...
           EndIf
            ...
            _cRetorno += Strzero((SE2->E2_SALDO-SE2->E2_ZZVLENT)*100,14)                               
            
            // Posicao 058 a 071: Valor Outras Entidades
            _cRetorno += Strzero(SE2->E2_ZZVLENT*100,14)            
            
            // Posicao 072 a 085: Multa        
            _cRetorno += Strzero(SE2->E2_ZZMULTA+SE2->E2_ZZJUROS*100,14)            
         ...
         ElseIf SEA->EA_MODELO == "22" //--- GARE ICMS - SP
            ...
            // Posicao 025 a 038: N Inscricao  //--- CNPJ/CPF do Contribuinte
            //_cRetorno +=  Strzero(Val(u_dadosSM0("02",SE2->E2_FILIAL)),14) // Comentado por Waldir - Funcao u_dadosSM0 está no fonte PAGAR356.PRW                               
            
            _cRetorno +=  Strzero(Val(RetField('SM0',1,SM0->M0_CODIGO+SE2->E2_FILIAL,'M0_CGC')),14) //--- Funcao u_dadosSM0 está no fonte PAGAR356.PRW
  
            //--- Posicao 039 a 050: Inscricao Estadual-  
            //_cRetorno +=  Strzero(Val(u_dadosSM0("01",SE2->E2_FILIAL)),12)   //Comentado por Waldir - Funcao u_dadosSM0 está no fonte PAGAR356.PRW                              
              
 			_cRetorno +=  Strzero(Val(RetField('SM0',1,SM0->M0_CODIGO+SE2->E2_FILIAL,'M0_INSC')),12)   //--- Funcao u_dadosSM0 está no fonte PAGAR356.PRW                              
 
 
            //--- Posicao 051 a 063: Divida Ativa / Etiqueta  
            _cRetorno +=  Strzero(Val(SE2->E2_ZZDIVID),13)                               

            //--- Posicao 064 a 069: Periodo de Referencia (Mes/Ano)  Formato MMAAAA 
            _cRetorno += Strzero(Month(SE2->E2_ZZAPURA),2)+Strzero(Year(SE2->E2_ZZAPURA),4)  

            //--- Posicao 070 a 082: N. Parcela / Notificação  
            _cRetorno +=  Strzero(Val(SE2->E2_ZZPARCE),13)                               
         ...
         ElseIf SEA->EA_MODELO == "25"  .or. SEA->EA_MODELO == "26" .or. SEA->EA_MODELO == "27" 
              ...
              _cRetorno += Strzero(SE2->E2_ZZANOBA,4)                               

              //--- Posicao 043 a 051 - Renavam  
              _cRetorno +=  Strzero(Val(SE2->E2_ZZRENAV),9)                               
                                                                           
              //--- Posicao 052 a 053 - Unidade Federação   
             _cRetorno +=  Upper(SE2->E2_ZZIPVAU)                               

             //--- Posicao 054 a 058 - Codigo do Municipio  
             _cRetorno += Strzero(Val(SE2->E2_ZZCDMUN),5)    

             //--- Posicao 059 a 065 - Placa   
             _cRetorno +=  SE2->E2_ZZPLACA                              

             //--- Posicao 066 a 066 - Opção de Pagamento - Para DPVAT sempre opção 0
             If SEA->EA_MODELO == "25"
                _cRetorno += Alltrim(SE2->E2_ZZOPCAO)
            Else
         ...
         ElseIf SEA->EA_MODELO == "35" 
            ...
            _cRetorno += Strzero(Val(SE2->E2_ZZIDENT),16)                           
            
            // Posicao 103 a 111: Lacre de Conectividade Social
            _cRetorno += Strzero(Val(SE2->E2_ZZLACRE),9)
             
            // Posicao 112 a 113: Digito do Lacre de Conectividade Social
            _cRetorno += Strzero(Val(SE2->E2_ZZDLACR),2)
            ...
            _cRetorno += STRZERO((SE2->E2_SALDO+SE2->E2_ZZMULTA+SE2->E2_ZZJUROS-SE2->E2_DECRESC)*100,14)           

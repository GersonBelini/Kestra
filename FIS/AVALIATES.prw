...
Local oSecTES := oReport:Section(2)
	...
	While !EOF()
		...
		If TRB->( F4_ICM == 'N' .AND. F4_LFICM != 'N')
		  	TRB->( GeraCell(oSecTES,"ERRO",F4_CODIGO,"F4_ICM",F4_ICM,"S","Para considerar o Livro, Calcula ICMS deve ser [SIM]") )
		Endif
	...
	//Cred PIS COFINS Saida x CSTPIS
		...
		If TRB->( F4_PISCRED != '3' .AND. F4_CSTPIS $'06' )
		  	TRB->( GeraCell(oSecTES,"ERRO",F4_CODIGO,"F4_PISCRED",F4_PISCRED,"3 - Calcula","CST PIS 06, Cred PIS/COF deve ser [Calcula]") )		
		Endif	
		If TRB->( F4_PISCRED != '4' .AND. F4_CSTPIS $'07|08|09' )
		  	TRB->( GeraCell(oSecTES,"ERRO",F4_CODIGO,"F4_PISCRED",F4_PISCRED,"4 - Nao Calcula","CST PIS 07,08 ou 09, Cred PIS/COF deve ser [Nao Calcula]") )		
		Endif	
